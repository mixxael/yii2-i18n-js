...
class I18nJs extends BaseObject
{
    ...
    {
        parent::init();
        $this->jsFilenameOnServer = Yii::getAlias('@webroot') . '/' . $this->jsFilename;
        $dirname = dirname($this->jsFilenameOnServer);
        if (!file_exists($dirname)) {
            mkdir($dirname, 0777, true);
        }
        $this->filenameForSavingModificationTime = Yii::getAlias('@runtime') . '/i18n-js-modification-time';
        ...
        Yii::$app->view->registerJsFile('@web/' . $this->jsFilename . '?v=' . $this->currentModificationTime);
    ...
    public static function getBasePaths()
    {
        ...
        foreach (Yii::$app->i18n->translations as $category => $translation) {
            if (!preg_match('=^yii($|/)=', $category)) {
                if (is_array($translation)) {
                    $basePaths[] = isset($translation['basePath'])
    ...
    private function getFilenames()
    {
        ...
        foreach ($this->basePaths as $basePath) {
            foreach (PhpHelper::getFilesInDirectory($basePath, true, ['php']) as $filename) {
                if (preg_match(
    ...
    private function getSavedModificationTime()
    {
        ...
        if (file_exists($this->filenameForSavingModificationTime)) {
            $modificationTime = (int) file_get_contents($this->filenameForSavingModificationTime);
        }
    ...
    private function saveJsFile()
    {
        ...
        foreach ($this->basePaths as $basePath) {
            foreach ($this->filenames as $filename) {
                $languageAndCategoryAsString = str_replace($basePath . DIRECTORY_SEPARATOR, '', $filename);
                // Delete file extension (.php)
                $languageAndCategoryAsString = substr($languageAndCategoryAsString, 0, -4);

                $languageAndCategoryAsArray = explode(DIRECTORY_SEPARATOR, $languageAndCategoryAsString, 2);
                $result
        ...
        return file_put_contents(
            ...
            "\n" . 'const YII2_I18N_JS = ' . Json::encode($result) . "\n"
        );
    }

    private function saveModificationTime()
    {
        file_put_contents($this->filenameForSavingModificationTime, $this->currentModificationTime . "\n");
    }
...
(_ => {
  ...
  if (!('t' in window.yii)) {
    if (!document.documentElement.lang) {
      throw new Error('You must specify the "lang" attribute for the <html> element')
    }
    yii.t = (category, message, params, language) => {
      ...
      if (
        ...
        !YII2_I18N_JS ||
        !YII2_I18N_JS[language] ||
        !YII2_I18N_JS[language][category] ||
        !YII2_I18N_JS[language][category][message]
      ) {
        translatedMessage = message
      } else {
        translatedMessage = YII2_I18N_JS[language][category][message]
      }
      if (params) {
        Object.keys(params).map(key => {
          // https://stackoverflow.com/a/6969486/4223982
          const escapedParam = key.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, '\\$&')
          translatedMessage = translatedMessage.replace(new RegExp('\\\{' + escapedParam + '\\\}', 'g'), params[key])
        })
      }
